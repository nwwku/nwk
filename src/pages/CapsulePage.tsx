...
export function CapsulePage() {
  ...
  useEffect(() => { if (!user) return; void Promise.all([loadWardrobe(gender), loadCapsuleIds()]).then(([wardrobe, ids]) => { setItems(wardrobe); setSelected(ids.filter((id) => wardrobe.some((item) => item.id === id))); setWardrobePromptOpen(wardrobe.length === 0); }).catch(() => setMessage(tr('Could not load the capsule.', 'Не удалось загрузить капсулу.'))); }, [gender, user]);
  ...
  return <div className="stack-lg fade-in"><section className="page-title"><div><p className="eyebrow">{selected.length}/15 {tr('pieces', 'вещей')}</p><h1>{tr('Your capsule', 'Твоя капсула')}</h1></div><span className="score-ring">{combinations}</span></section><p className="page-intro">{tr('Choose versatile pieces that work together. The number shows possible pairings.', 'Выбери универсальные вещи, которые сочетаются между собой. Число показывает возможные пары.')}</p>{message && <p className="form-error">{message}</p>}
    ...
    {!items.length && <div className="empty-state"><Icon name="hanger" /><p>{tr('Your wardrobe is empty.', 'Твой гардероб пока пуст.')}</p></div>}
    {wardrobePromptOpen && <WardrobeRequiredDialog action="capsule" onClose={() => setWardrobePromptOpen(false)} />}
  </div>;
}
