...
export function CreatePage() {
  ...
  const shoppingQuery = new URLSearchParams({ style });
  const requestedImage = params.get('image');
  if (requestedImage?.startsWith('/assets/')) shoppingQuery.set('image', requestedImage);
  const wardrobePrompt = emptyWardrobe ? <WardrobeRequiredDialog action="outfit" secondaryHref={`/look-shop?${shoppingQuery.toString()}`} onClose={() => setEmptyWardrobe(false)} /> : null;

  useEffect(() => { if (!user) { setWardrobeLoading(false); return; } setWardrobeLoading(true); void loadWardrobe(gender).then(setItems).catch(() => setError(tr('Could not load your wardrobe.', 'Не удалось загрузить гардероб.'))).finally(() => setWardrobeLoading(false)); }, [gender, user]);
  async function create(nextStyle = style) {
    if (userLoading || wardrobeLoading) return;
